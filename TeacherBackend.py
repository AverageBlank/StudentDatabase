...
def ClassRecords(Class=None):
    ...
    if Class in [11, 12]:
        ...
        if len(MPCResult) != 0:
            ...
            df1 = dataframe(MPCResult).style.hide(axis="columns")
        # ? BiPC
        if len(BiPCResult) != 0:
            df2 = dataframe(BiPCResult)
        else:
            df2 = dataframe(BiPCResult).style.hide(axis="columns")
        # ? Commerce
        if len(CommerceResult) != 0:
            df3 = dataframe(CommerceResult)
        else:
            df3 = dataframe(CommerceResult).style.hide(axis="columns")
        # ? Humanities
        if len(HumanitiesResult) != 0:
            df4 = dataframe(HumanitiesResult)
        else:
            df4 = dataframe(HumanitiesResult).style.hide(axis="columns")

        # ? Uploading code
        with open(f"Class {Class} Record.html", "w") as f:
            f.write(
                '<head><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KK94CHFLLe+nY2dmCWGMq91rCGa5gtU4mk92HdvYe+M/SXH301p5ILy+dN9+nJOZ" crossorigin="anonymous"></head></head>'
            )
            f.write(
